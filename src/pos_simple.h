#ifndef __RcppJaggerPOSSimple__INCLUDED__
#define __RcppJaggerPOSSimple__INCLUDED__
#include <Rcpp.h>
#include "reader.h"
...
class RcppJaggerPOSSimple : public jagger::tagger {
  public:
    // Constructor
    RcppJaggerPOSSimple() : jagger::tagger() {};

    // POS tagger  (need to change `c2i` etc from private to protected)
    template <const int BUF_SIZE_, const bool POS_TAGGING>
    void pos(const std::string &inputstring, std::vector<std::string> &token_vec, std::vector<std::string> &pos_vec) {
